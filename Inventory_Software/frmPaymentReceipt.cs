...
namespace Inventory_Software
{
    public partial class frmPaymentReceipt : Form
    {
        ...
        {
            cn.Open();
            da = new MySqlDataAdapter("SELECT c.invoiceno, c.pname, c.price, c.qty, c.total, p.total AS ptotal, p.amountpaid, p.mchange, p.paymode, p.date, p.time,c.user, c.status FROM tblcart AS c INNER JOIN tblpayment AS p ON c.invoiceno = p.invoiceno WHERE c.invoiceno = '" + invoiceno + "'", cn);
        ...
        public void LoadHeader()
        {
            ...
            if (dr.HasRows)
            {
                ...
                reportViewer1.LocalReport.ReportPath = Application.StartupPath + "/Reports/PaymentReceipt.rdlc";
